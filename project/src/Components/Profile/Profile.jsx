...
const USER_API_URL = "http://10.105.227.237:5001/api";
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        const userResponse = await axios.get(`${USER_API_URL}/user/me`, {
          withCredentials: true,
        });
        const userData = userResponse.data.user;
        setEditForm({ username: userData.username, email: userData.email });
        setdetails({
          Authetication: userData.authType,
          createdAt: userData.createdAt,
          picture: userData.picture,
        });

        // Fetch vehicles from Vehicle Service
        const vehiclesResponse = await axios.get(`${VEHICLE_API_URL}/vehicles`, {
          withCredentials: true,
        });
        setVehicles(vehiclesResponse.data);

        // Fetch appointments from Appointment Service
        const appointmentsResponse = await axios.get(`${APPOINTMENT_API_URL}/appointments`, {
          withCredentials: true,
        });
