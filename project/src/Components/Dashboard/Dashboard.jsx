...
const Dashboard = () => {
  ...
  // Fetch vehicles from Vehicle microservice
  useEffect(() => {
    const fetchVehicles = async () => {
      if (!user) return;
      setLoading(true);
      try {
        const response = await axios.get(`${VEHICLE_API_URL}/vehicles`, { withCredentials: true });
  ...
  // Fetch appointments from Appointment microservice
  useEffect(() => {
    const fetchAppointments = async () => {
      if (!user) return;
      try {
        const response = await axios.get(`${APPOINTMENT_API_URL}/appointments`, { withCredentials: true });
  ...
  const handleVehicleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${VEHICLE_API_URL}/vehicles`, vehicleForm, { withCredentials: true });
  ...
  const confirmDeleteVehicle = async () => {
    ...
    try {
      await axios.delete(`${VEHICLE_API_URL}/vehicles/${vehicleToDelete}`, { withCredentials: true });
  ...
  const handleAppointmentSubmit = async (e) => {
    ...
    try {
      const response = editingAppointment
        ? await axios.put(`${APPOINTMENT_API_URL}/appointments/${editingAppointment._id}`, appointmentForm, {
            withCredentials: true,
          })
        : await axios.post(`${APPOINTMENT_API_URL}/appointments`, appointmentForm, { withCredentials: true });
  ...
  const confirmDeleteAppointment = async () => {
    ...
    try {
      ...
      await axios.delete(`${APPOINTMENT_API_URL}/appointments/${appointmentToDelete}`, { withCredentials: true });
